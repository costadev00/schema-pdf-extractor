...
def load_pdf_bytes(pdf_input) -> Tuple[Optional[bytes], Optional[str]]:
    if pdf_input is None:
        return None, "Nenhum PDF fornecido."

    # Gradio may return different structures depending on version/component
    ...
    for path in potential_paths:
        if path and os.path.exists(path):
            try:
                ...
                return None, f"Erro ao ler PDF: {exc}"
    ...
    return None, "Não foi possível acessar os bytes do PDF enviado."
...
def run_pipeline(
    ...
    pdf_bytes, pdf_error = load_pdf_bytes(pdf_file)
    if pdf_error is not None:
        return json.dumps({}, ensure_ascii=False, indent=2), pdf_error
    if not pdf_bytes:
        return json.dumps({}, ensure_ascii=False, indent=2), "PDF vazio ou não lido."
